...
const RPC_URLS: &[&str] = &[
    "http://127.0.0.1:9650/ext/bc/C/rpc",          // ONLY: local go-flare (<1ms, fully synced)
    // Fallbacks deaktiviert: externe RPCs erzeugen "Unknown network" bei Rotation
];

const WS_URLS: &[&str] = &[
    "ws://127.0.0.1:9650/ext/bc/C/ws",              // ONLY: local go-flare WS
];

// ── Gas Strategy (dynamisch — eth_gasPrice + 50% Aufschlag) ─────────────────
//
// Holt gas_price live vom Node und addiert 50% Aufschlag (Tommy: aggressiv!).
// Env-Overrides:
//   KOJO_GAS_SURCHARGE_PCT=50      (default: 50% für Arb)
//   KOJO_FASSETS_GAS_SURCHARGE_PCT=60  (default: 60% für FAssets — noch aggressiver)
//   KOJO_GAS_CAP_GWEI=50000        (default: 50000 Gwei Cap — Spike-Schutz)
//   KOJO_GAS_FLOOR_GWEI=5          (default: 5 Gwei Minimum)

const GWEI: u64 = 1_000_000_000;

async fn fetch_dynamic_gas_price(provider: &Arc<Provider<Http>>, surcharge_pct: u64) -> U256 {
    let floor_gwei = env::var("KOJO_GAS_FLOOR_GWEI")
        .ok().and_then(|s| s.parse::<u64>().ok()).unwrap_or(5);
    let cap_gwei = env::var("KOJO_GAS_CAP_GWEI")
        .ok().and_then(|s| s.parse::<u64>().ok()).unwrap_or(50_000);

    let floor = U256::from(floor_gwei * GWEI);
    let cap   = U256::from(cap_gwei * GWEI);

    match provider.get_gas_price().await {
        Ok(base) => {
            let surcharge = base * U256::from(surcharge_pct) / U256::from(100u64);
            let price = (base + surcharge).max(floor).min(cap);
            let base_gwei  = base.as_u128() as f64 / GWEI as f64;
            let final_gwei = price.as_u128() as f64 / GWEI as f64;
            tracing::debug!("GAS: base={:.2} Gwei +{}% → {:.2} Gwei", base_gwei, surcharge_pct, final_gwei);
            price
        }
        Err(e) => {
            tracing::warn!("GAS: eth_gasPrice fehlgeschlagen ({}), nutze Floor {} Gwei", e, floor_gwei);
            floor
        }
    }
}
...
async fn notify_discord_kojo_trade(
    label: &str,
    profit_usd: f64,
    tx_hash: H256,
) {
...
// BlazeSwap Factory V2 — (Uniswap V2 Fork)
const BLAZESWAP_FACTORY: &str = "0xC526c8022Ea1D7b63f739BB8E72877E9A96E7599"; // TODO: Verify address

// ── KOJO FLARE ENGINE: BLAZESWAP STRATEGIES ───────────────────────────────────

pub mod kojo_flare_engine {
    pub mod strategies {
        pub mod blaze_titan {
            use tracing::info;
            // Blue-Chip Sniper
            pub const TRIGGER_DELTA_PCT: f64 = 0.15;
            
            pub fn evaluate(pool: &str, ftso_delta: f64) -> bool {
                let valid_pools = ["WFLR/USDCE", "WFLR/USDT", "FXRP/WFLR", "SFLR/WFLR"];
                if !valid_pools.contains(&pool) { return false; }
                
                if ftso_delta.abs() > TRIGGER_DELTA_PCT {
                    info!("BLAZE-TITAN [Blue-Chip Sniper] Triggered on {}! Delta: {:.2}% (> 0.15%). Execution: Max gas priority, dyn size (slip 0.05%)", pool, ftso_delta);
                    return true;
                }
                false
            }
        ...
        pub mod blaze_ghost {
            ...
            pub const TRIGGER_DIVERGENCE_PCT: f64 = 2.00;
            pub const MIN_LIQUIDITY_USD: f64 = 500.0;
            
            pub fn evaluate(pool: &str, divergence: f64, pool_liquidity_usd: f64) -> bool {
                let titan_pools = ["WFLR/USDCE", "WFLR/USDT", "FXRP/WFLR", "SFLR/WFLR"];
                if titan_pools.contains(&pool) { return false; } // Handled by Titan
                
                if pool_liquidity_usd < MIN_LIQUIDITY_USD { return false; }
                
                if divergence.abs() > TRIGGER_DIVERGENCE_PCT {
                    info!("BLAZE-GHOST [High-Vol Hunter] Triggered on {}! Divergence: {:.2}% (> 2.00%). Execution: Fixed small size, aggressive sniping", pool, divergence);
                    return true;
                }
                false
            }
...
const FTSO_FAST_UPDATER: &str = "0xdBF71d7840934EB82FA10173103D4e9fd4054dd1"; // Flare Mainnet Active FastUpdater (for submitUpdates)
const FTSO_V2_PUBLIC: &str = "0x7BDE3Df0624114eDB3A67dFe6753e62f4e7c1d20"; // FtsoV2 Consumer Interface (for getFeedsById)
...
const POOLS: &[(&str, &str, &str, u32)] = &[
    // Enosys V2 Pools (Uniswap V2)
    ("WFLR/USDCE", "Enosys-V2", "0xb1ec7ef55fa2e84eb6ff9ff0fa1e33387f892f68", 3000),
    ("WFLR/SFLR",  "Enosys-V2", "0x3F50F880041521738fa88C46cDF7e0d8Eeb11Aa2", 3000),
    ("FXRP/USDT0", "Enosys-V2", "0x3D6EFe2e110F13ea22231be6B01B428B38CafC92", 3000), // token0=FXRP(6dec) token1=USDT0(6dec) ~112k/160k Liq
    
    // Enosys V3 Pools (Algebra)
    ("WFLR/USDCE", "Enosys-V3", "0x9af682206Ee6f6651cb09ca0DECd169B2E861D06", 3000),
];
...
// SparkDEX/UniV3 Pool — Standard UniV3 slot0
...
// FTSOv2 Public Consumer — getFeedsById für Preis-Feeds
abigen!(IFtsoV2, r#"[
...
const FTSO_TOP_50: &[&str] = &[
    "FLR", "XRP", "BTC", "ETH", "ADA", "SOL", "MATIC", "DOGE", "DOT", "AVAX",
    "LTC", "LINK", "XLM", "ALGO", "ARB", "FIL", "SGB", "BNB", "USDC", "USDT",
    "XDC", "BCH", "UNI", "ATOM", "TON", "ICP", "APT", "NEAR", "OP", "INJ",
    "AAVE", "QNT", "MKR", "SNX", "GRT", "LDO", "STX", "IMX", "VET", "RNDR",
    "FTM", "MNT", "AR", "CRV", "THETA", "EGLD", "PEPE", "HBAR", "SAND", "EOS"
];

fn get_feed_id(symbol: &str) -> [u8; 21] {
    let mut id = [0u8; 21];
    id[0] = 0x01;
    let name = format!("{}/USD", symbol);
    let bytes = name.as_bytes();
    let len = std::cmp::min(bytes.len(), 20);
    id[1..1+len].copy_from_slice(&bytes[..len]);
    id
}
...
async fn fetch_ftso_prices(provider: Arc<Provider<Http>>, block: u64) -> Option<FtsoSnapshot> {
    let addr: Address = FTSO_V2_PUBLIC.parse().ok()?;
    let ftso = IFtsoV2::new(addr, provider);

    let mut feed_ids: Vec<[u8; 21]> = Vec::new();
    for symbol in FTSO_TOP_50 {
        feed_ids.push(get_feed_id(symbol));
    }
    ...
    if values.len() != FTSO_TOP_50.len() { return None; }
    ...
    for (i, symbol) in FTSO_TOP_50.iter().enumerate() {
        let price = to_f64(values[i], decimals[i]);
...
async fn fetch_price(
    ...
    let addr: Address = match pool_addr.parse() {
        Ok(a) => a,
        Err(e) => {
            warn!("DEX PRICE FAIL: {} {} — bad address: {}", dex_name, pair, e);
            return None;
        }
    };
    
    let price = if dex_name.contains("V2") || dex_name == "Enosys-V2" {
        // ── Enosys V2: getReserves ──
        let pool = IUniswapV2Pair::new(addr, provider.clone());
        let reserves = match timeout(Duration::from_secs(2), pool.get_reserves().call()).await {
            Ok(Ok(r)) => r,
            Ok(Err(e)) => {
                warn!("DEX PRICE FAIL: {} {} — getReserves reverted: {}", dex_name, pair, e);
                return None;
            }
            Err(_) => {
                warn!("DEX PRICE FAIL: {} {} — getReserves timeout", dex_name, pair);
                return None;
            }
        };
        let (r0, r1, _) = reserves;
        info!("DEX RAW: {} {} — r0={} r1={}", dex_name, pair, r0, r1);
        
        // Token order matters! Verified on-chain:
        // WFLR/USDCE: token0=WFLR(18dec), token1=USDCE(6dec)
        // WFLR/SFLR:  token0=sFLR(18dec), token1=WFLR(18dec)
        match pair {
            "WFLR/USDCE" => (r1 as f64 / 1e6) / (r0 as f64 / 1e18),
            "WFLR/SFLR"  => (r0 as f64) / (r1 as f64),  // sFLR/WFLR ratio
            "FXRP/USDT0" => (r1 as f64) / (r0 as f64),  // USDT0/FXRP (beide 6dec → kein Scale nötig)
            _            => (r1 as f64) / (r0 as f64),
        }
    } else if dex_name.contains("Enosys-V3") {
        // ── Enosys V3 Algebra: globalState() raw call ──
        let tx_req = TransactionRequest::new()
            .from(Address::zero())
            .to(addr)
            .gas(500000)
            .data(ethers::types::Bytes::from(vec![0xe7, 0x6c, 0x01, 0xe4])); // globalState() — correct selector!
        let tx: ethers::types::transaction::eip2718::TypedTransaction = tx_req.into();
        
        let res = match timeout(Duration::from_secs(2), provider.call(&tx, None)).await {
            Ok(Ok(r)) => r,
            Ok(Err(e)) => {
                warn!("DEX PRICE FAIL: {} {} — globalState reverted: {}", dex_name, pair, e);
                return None;
            }
            Err(_) => {
                warn!("DEX PRICE FAIL: {} {} — globalState timeout", dex_name, pair);
                return None;
            }
        };
            
        if res.len() < 64 {
            warn!("DEX PRICE FAIL: {} {} — globalState too short: {} bytes", dex_name, pair, res.len());
            return None;
        }
        
        let sqrt_raw = U256::from_big_endian(&res[0..32]);
        info!("DEX RAW: {} {} — sqrtPriceX96={}", dex_name, pair, sqrt_raw);
        let sqrt = sqrt_raw.as_u128() as f64 / 2_u128.pow(96) as f64;
        
        // token0=WFLR(18dec), token1=USDCE(6dec)
        match pair {
            "WFLR/USDCE" => sqrt.powi(2) * 1e12,
            "WFLR/SFLR"  => sqrt.powi(2),
            _            => sqrt.powi(2),
        }
    } else {
        // ── Standard UniV3: slot0() ──
        let pool = IUniswapV3Pool::new(addr, provider);
        let slot0 = match timeout(Duration::from_secs(2), pool.slot_0().call()).await {
            Ok(Ok(s)) => s,
            Ok(Err(e)) => {
                warn!("DEX PRICE FAIL: {} {} — slot0 reverted: {}", dex_name, pair, e);
                return None;
            }
            Err(_) => {
                warn!("DEX PRICE FAIL: {} {} — slot0 timeout", dex_name, pair);
                return None;
            }
        };
        let sqrt_raw = slot0.0;
        let sqrt = sqrt_raw.as_u128() as f64 / 2_u128.pow(96) as f64;
    ...
    info!("DEX PRICE: {} {} = {:.8} (fee={}bps)", dex_name, pair, price, fee / 100);

    let valid = match pair {
        "WFLR/USDCE" => price > 0.001 && price < 1.0,
        "WFLR/SFLR"  => price > 0.5   && price < 2.0,
        "FXRP/USDT0" => price > 0.1   && price < 100.0,
        _            => price > 0.0,
    };

    if !valid {
        warn!("DEX PRICE INVALID: {} {} = {:.8} — out of sanity range", dex_name, pair, price);
        return None;
    }
...
async fn execute_simple_arb<M: Middleware + 'static>(
    ...
    // Dynamisches Gas: eth_gasPrice + 50% Aufschlag (Tommy: aggressiv, kein Spike-Ausfall!)
    let arb_surcharge = env::var("KOJO_GAS_SURCHARGE_PCT").ok().and_then(|s| s.parse().ok()).unwrap_or(50u64);
    let _rpc = env::var("CUSTOM_RPC_URLS").or_else(|_| env::var("RPC_URL")).unwrap_or_else(|_| "http://127.0.0.1:9650/ext/bc/C/rpc".to_string());
    let _rpc = _rpc.split(',').next().unwrap_or("http://127.0.0.1:9650/ext/bc/C/rpc").trim().to_string();
    let _gas_provider = Arc::new(Provider::<Http>::try_from(_rpc.as_str()).unwrap_or_else(|_| Provider::<Http>::try_from("http://127.0.0.1:9650/ext/bc/C/rpc").unwrap()));
    let gas_price = fetch_dynamic_gas_price(&_gas_provider, arb_surcharge).await;
    info!("ARB GAS: {:.2} Gwei (+{}%)", gas_price.as_u128() as f64 / 1_000_000_000.0, arb_surcharge);
    match flashbot
        ...
        Ok(pending) => {
            ...
            match pending.await {
                Ok(Some(r)) => {
                    ...
                    info!("CONFIRMED block #{} gas={}", r.block_number.unwrap_or_default(), gas_used);
                    // Grobe PnL-Schätzung basierend auf Trade-Größe und Net-Basis-Punkten.
                    let est_profit_usd =
                        (max_trade_usd as f64) * (net_bps as f64) / 10_000.0;
                    tokio::spawn(notify_discord_kojo_trade(
                        "DEX ARB",
                        est_profit_usd,
                        tx_hash,
                    ));
...
async fn execute_fassets_liq<M: Middleware + 'static>(
    ...
    // Dynamisches Gas: eth_gasPrice + 60% Aufschlag (FAssets = Zeit-kritisch, Backrun!)
    let liq_surcharge = env::var("KOJO_FASSETS_GAS_SURCHARGE_PCT").ok().and_then(|s| s.parse().ok()).unwrap_or(60u64);
    let _rpc2 = env::var("CUSTOM_RPC_URLS").or_else(|_| env::var("RPC_URL")).unwrap_or_else(|_| "http://127.0.0.1:9650/ext/bc/C/rpc".to_string());
    let _rpc2 = _rpc2.split(',').next().unwrap_or("http://127.0.0.1:9650/ext/bc/C/rpc").trim().to_string();
    let _gas_provider2 = Arc::new(Provider::<Http>::try_from(_rpc2.as_str()).unwrap_or_else(|_| Provider::<Http>::try_from("http://127.0.0.1:9650/ext/bc/C/rpc").unwrap()));
    let gas_price = fetch_dynamic_gas_price(&_gas_provider2, liq_surcharge).await;
    info!("FASSETS GAS: {:.2} Gwei (+{}%)", gas_price.as_u128() as f64 / 1_000_000_000.0, liq_surcharge);
    match flashbot
        ...
        .await
    {
        Ok(pending) => {
            let tx_hash = pending.tx_hash();
            info!("FASSETS LIQ TX: {:?}", tx_hash);
            match pending.await {
                Ok(Some(r)) => {
                    let gas_used = r.gas_used.unwrap_or_default();
                    info!("LIQ CONFIRMED block #{} gas={}", r.block_number.unwrap_or_default(), gas_used);
                    tokio::spawn(notify_discord_kojo_trade(
                        "FASSETS LIQ",
                        opp.est_profit_usd,
                        tx_hash,
...
async fn main() -> Result<()> {
    ...
    let min_profit_usd = env::var("MIN_PROFIT_USD").ok().and_then(|s| s.parse().ok()).unwrap_or(0.0001f64);
    let max_trade_usd  = env::var("MAX_TRADE_SIZE_USD").ok().and_then(|s| s.parse().ok()).unwrap_or(100u64);
    ...
    let signing_addr = first_wallet.address();

    let client = Arc::new(SignerMiddleware::new(rotator.get().as_ref().clone(), first_wallet.clone()));
    let mut flashbot = IFlashBotFlare::new(contract_addr.parse::<Address>()?, client.clone());
            ...
            while let Some(block) = stream.next().await {
                ...
                if enable_fast_submit {
                    ...
                    if let Some(payload) = payload_opt {
                        ...
                        let fast_updater_with_client = FastUpdaterSubmitClient::new(fast_updater.address(), client.clone());
                        match send_fast_update_submission(&fast_updater_with_client, payload, client.signer()).await {
                            Ok(Some(_)) => {}
                            Ok(None) => {}
                ...
                if let Some(ref f) = ftso {
                    ...
                    for opp in &liq_opps {
                        ...
                        // Liquidations-Betrag: dynamisch nach max_trade_usd Startgrösse
                        let liq_amount = U256::from(max_trade_usd * 1_000_000); // USDC.e-skaliert
                ...
                // ── BLAZESWAP TITAN & GHOST STRATEGIES ──────────────────────────
                if let (Some(ref f), Some(ref p)) = (&ftso, &prev_ftso) {
                    if p.flr_usd > 0.001 {
                        let flr_delta = (f.flr_usd - p.flr_usd) / p.flr_usd * 100.0;
                        for price in &prices {
                            if price.dex_name == "BlazeSwap" {
                                if kojo_flare_engine::strategies::blaze_titan::evaluate(&price.pair, flr_delta) {
                                    if live_trading && last_trade.elapsed() >= Duration::from_secs(3) {
                                        info!("🔥 BLAZE-TITAN EXECUTION: Initiating Titan Snipe on {}!", price.pair);
                                        let call_tx = flashbot.call_contract(
                                            H160::zero(),      // Placeholder target contract
                                            Bytes::new(),      // Placeholder payload
                                            H160::zero()       // Placeholder token (WFLR default usually)
                                        );
                                        let result = timeout(
                                            Duration::from_secs(2),
                                            call_tx.send()
                                        ...
                                        match result {
                                            Ok(Ok(tx)) => {
                                                info!("🔥 BLAZE-TITAN EXECUTION SUCCESS: Tx 0x{:x}", tx.tx_hash());
                                                // TODO: discord notification here
                                            }
                                            Ok(Err(e)) => error!("🔥 BLAZE-TITAN EXECUTION FAILED: {:?}", e),
                                            Err(_) => error!("🔥 BLAZE-TITAN EXECUTION TIMEOUT"),
                                        }
                                        trades_ok += 1;
                                        last_trade = Instant::now();
                                    } else if !live_trading {
                                        info!("🔥 BLAZE-TITAN DRY-RUN: Would execute Titan Snipe on {}", price.pair);
                                    }
                                }
                                let mock_divergence = flr_delta * 1.8; 
                                let mock_liquidity_usd = 800.0;
                                if kojo_flare_engine::strategies::blaze_ghost::evaluate(&price.pair, mock_divergence, mock_liquidity_usd) {
                                    if live_trading && last_trade.elapsed() >= Duration::from_secs(3) {
                                        info!("👻 BLAZE-GHOST EXECUTION: Initiating Ghost Snipe on {}!", price.pair);
                                        let call_tx = flashbot.call_contract(
                                            H160::zero(),      // Placeholder target contract
                                            Bytes::new(),      // Placeholder payload
                                            H160::zero()       // Placeholder token (WFLR default usually)
                                        ...
                                        match result {
                                            Ok(Ok(tx)) => {
                                                info!("👻 BLAZE-GHOST EXECUTION SUCCESS: Tx 0x{:x}", tx.tx_hash());
                                                // TODO: discord notification here
                                            }
                                            Ok(Err(e)) => error!("👻 BLAZE-GHOST EXECUTION FAILED: {:?}", e),
                                            Err(_) => error!("👻 BLAZE-GHOST EXECUTION TIMEOUT"),
                                        }
                                        trades_ok += 1;
                                        last_trade = Instant::now();
                                    } else if !live_trading {
                                        info!("👻 BLAZE-GHOST DRY-RUN: Would execute Ghost Snipe on {}", price.pair);
                                    }
                                }
                            }
                        }
                ...
                // rotate() deaktiviert — nur lokaler Node (kein chain_id mismatch)
            ...
            loop {
                ...
                if enable_fast_submit {
                    ...
                    if let Some(payload) = payload_opt {
                        let client = wallet_rotator.get_next();
                        let fast_updater_with_client = FastUpdaterSubmitClient::new(fast_updater.address(), client.clone());
                        if let Err(e) = send_fast_update_submission(&fast_updater_with_client, payload, client.signer()).await {
